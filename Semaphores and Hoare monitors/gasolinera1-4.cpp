...
const unsigned total_coches = 10;
const unsigned C = 3; //nº surtidores diésel
const unsigned D = 2; //nº surtidores gasolina

//Variable global
unsigned int surtidores_en_uso = 0; //como máx debería valer C+D

//**********************************************************************
//Semáforos
Semaphore surtidores_diesel = C;
Semaphore surtidores_gasolina = D;
Semaphore acceso_contador = 1; //va a funcionar como un lock
...
void funcion_hebra_diesel (int n_coche){
    while (true){
        ...
        sem_wait (acceso_contador);
        surtidores_en_uso ++;
        cout << "El número de surtidores en uso ahora es " << surtidores_en_uso << endl;
        sem_signal (acceso_contador);
