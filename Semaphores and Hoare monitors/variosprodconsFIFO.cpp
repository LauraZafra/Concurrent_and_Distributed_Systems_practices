...
/*
REVISAR PARÁMETROS PRODUCIR DATO Y CONSUMIR DATO
#include <iostream>
#include <cassert>
#include <thread>
#include <mutex>
...
//Semaforos para la exclusión mútua, con solo 1 prod y 1 cons no los necesitabamos
Semaphore puede_acceder_p = 1;
Semaphore puede_acceder_c = 1;
...
void  funcion_hebra_productora(  )
{
    ...
    { //lo de dentro del for es nuestro
        ...
        sem_wait(puede_acceder_p); // necesario al haber varios productores

        buffer[primera_libre] = dato;
        primera_libre++;
        primera_libre = primera_libre%tam_vec; //nos aseguramos de no irnos fuera del vector

        sem_signal(puede_acceder_p); // necesario al haber varios productores
...
void funcion_hebra_consumidora(  )
{
    ...
    { //lo de dentro del for es nuestro
        ...
        sem_wait(puede_acceder_c);  // necesario al haber varios consumidores

        dato = buffer[primera_ocupada];
        primera_ocupada++;
        primera_ocupada = primera_ocupada%tam_vec; //nos aseguramos de no irnos fuera del vector

        sem_signal(puede_acceder_c);  // necesario al haber varios consumidores
...
int main(){
    ...
    cout << "-----------------------------------------------------------------" << endl
         << "Problema de los productores-consumidores (varios) (solución FIFO)." << endl
         << "------------------------------------------------------------------" << endl
         << flush ;
    ...
    //esto nos asegura que terminen todas las hebras antes de finalizar el main
