...
/*
Si todos los filosofos cogen primero el tenedor izquierdo o el derecho, hay interbloqueo
*/
...
void funcion_filosofos( int id )
{
  ...
  while ( true )
  {
    ...
    //MPI_Ssend( void* buf_emi, int count, MPI_Datatype datatype, int dest, int tag, MPI_Comm comm );
...
int main( int argc, char** argv )
{
   ...
   if ( id_propio == 0 ) // solo el primero escribe titulo, indep. del rol
      {
         cout << "----------------------------------------------------------------------------" << endl;
         cout << "PROBLEMA CENA DE LOS FILÓSOFOS (CON INTERBLOQUEO) CON MPI (PASO DE MENSAJES)" << endl;
         cout << "----------------------------------------------------------------------------" << endl;
      } 
